...
~> Season  : *${season}*`});
		...
		console.log(e)
		reject(`Nao sei que anime é este`)
	}
})
...
const simih = async (text) => {
	try {
		...
		return 'Simi Eu nao entendo bem que voce fala escreve certo , língua estrangeira, poraa?'
	}
}
...
const randomNimek = async (type) => {
    ...
    switch(type) {
        ...
        case 'hentai':
            ...
            break
        case 'trap':
            let trap = await fetch(url + 'nsfwtrap')
...
const banner = cfonts.render(('TERMUX|BOT|WA'), {
    font: 'chrome',
    color: 'candy',
    align: 'center',
    gradient: ["red","yellow"],
...
module.exports = { wait, simih, getBuffer, h2k, generateMessageID, getGroupAdmins, randomNimek, getRandom, start, info, success, banner, close }
